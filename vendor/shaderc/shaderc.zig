...
pub const CompileOptions = struct {
    ...
    pub fn setIncludeCallbacks(self: @This(), resolve_fn: c.shaderc_include_resolve_fn, release_fn: c.shaderc_include_result_release_fn, context: ?*anyopaque) void {
        c.shaderc_compile_options_set_include_callbacks(self.handle, resolve_fn, release_fn, context);
    }
    ...
    pub fn setVersion(self: @This(), env_target: shaderc.Env.Target, env_version: shaderc.Env.VulkanVersion) void {
        c.shaderc_compile_options_set_target_env(self.handle, @intFromEnum(env_target), @intFromEnum(env_version));
    }
};
...
pub const Env = struct {
    ...
    pub const VulkanVersion = enum(c.shaderc_env_version) {
        ...
        @"gl45" = c.shaderc_env_version_opengl_4_5,
    };
};
